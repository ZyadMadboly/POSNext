    ...
    <template #body-content>
      <div class="space-y-3 md:space-y-6">
        ...
        <div v-else-if="closingData" class="space-y-3 md:space-y-6">
          ...
          <div class="bg-white border border-gray-200 rounded-lg p-3 md:p-6 shadow-sm">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
              ...
              <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 md:p-4">
                ...
                <div class="text-lg md:text-2xl font-bold text-gray-900 mb-0.5 md:mb-1 truncate">{{ formatCurrency(getTotalTax()) }}</div>
          ...
          <!-- No Sales Warning -->
          <div v-if="getInvoiceCount() === 0" class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 md:p-4">
          ...
          <div v-if="getInvoiceCount() > 0" class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <button
            ...
            >
              <div class="text-left">
                ...
                <p class="text-xs md:text-sm text-gray-500">{{ getInvoiceCount() }} transactions • {{ formatCurrency(closingData.grand_total) }}</p>
              </div>
              <svg
          ...
          <div class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <div class="bg-gray-50 px-3 py-3 md:px-6 md:py-4 border-b border-gray-200">
              <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <div>
                  <h3 class="text-sm md:text-lg font-semibold text-gray-900">Payment Reconciliation</h3>
                  <p class="text-xs md:text-sm text-gray-600">Count your cash and enter actual amounts below</p>
                </div>
                <div v-if="getTotalDifference !== 0" class="text-left sm:text-right">
              ...
              >
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3">
                  ...
                  <div class="bg-white rounded-lg p-2 md:p-3 border border-gray-300">
                    ...
                    />
                    <div class="text-xs text-gray-500 mt-0.5 md:mt-1 hidden sm:block">Count & enter</div>
                  </div>
                </div>

                <!-- Difference Alert -->
                <div v-if="payment.closing_amount !== null && payment.closing_amount !== undefined && payment.difference !== 0"
                     class="mt-2 md:mt-3 p-2 md:p-3 rounded-lg" :class="[
            ...
            <div class="bg-gray-50 px-3 py-3 md:px-6 md:py-4 border-t border-gray-200">
          ...
          <!-- Tax Summary -->
          <div v-if="closingData.taxes && closingData.taxes.length > 0" class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            ...
            <div class="p-3 md:p-6">
              ...
              <div class="mt-3 md:mt-4 pt-3 md:pt-4 border-t border-gray-200">
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-base md:text-lg font-bold text-gray-900">{{ formatCurrency(getTotalTax()) }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Error Display -->
          <div v-if="submitResource.error" class="rounded-lg bg-red-50 border border-red-200 p-3 md:p-4">
            <div class="flex gap-2 md:gap-3">
              ...
              <div>
                ...
                <p class="text-xs md:text-sm text-red-700 mt-1">{{ submitResource.error }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Error State -->
        <div v-else-if="closingDataResource.error" class="rounded-lg bg-red-50 border border-red-200 p-3 md:p-4">
          <div class="flex gap-2 md:gap-3">
            <svg class="h-4 w-4 md:h-5 md:w-5 text-red-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
            </svg>
            <div>
              ...
              <p class="text-xs md:text-sm text-red-700 mt-1">{{ closingDataResource.error }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <template #actions>
      <div class="flex flex-col sm:flex-row justify-between w-full items-stretch sm:items-center gap-2 sm:gap-0">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 order-1 sm:order-2">
          <!-- Validation Warning -->
          <div v-if="!canSubmit && closingData" class="text-xs md:text-sm text-yellow-600 font-medium text-center sm:text-right">
          ...
          <Button
            ...
            :disabled="!canSubmit"
          >
            {{ submitResource.loading ? 'Closing...' : 'Close Shift' }}
          </Button>
        </div>
      </div>
    </template>
