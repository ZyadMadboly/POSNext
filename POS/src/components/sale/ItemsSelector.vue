							...
							<h3 class="text-[10px] sm:text-xs font-semibold text-gray-900 truncate mb-0.5 leading-tight">
								{{ item.item_name }}
                                                        ...
                                                        <div class="text-[9px] sm:text-[10px] text-gray-500 leading-tight">
                                                                ...
                                                                <div v-if="item.rate && item.price_list_rate && item.rate !== item.price_list_rate" class="space-y-0.5">
                                                                        <p>
                                                                                <span class="font-semibold text-blue-600">{{ formatCurrency(item.rate) }}</span>
                                                                                <span class="text-gray-400">/ {{ item.uom || item.stock_uom || 'Nos' }}</span>
                                                                                <span class="text-xs text-gray-400 ml-1">(Current)</span>
                                                                        </p>
                                                                        <p>
                                                                                <span class="font-medium text-gray-500 line-through">{{ formatCurrency(item.price_list_rate) }}</span>
                                                                                <span class="text-xs text-gray-400 ml-1">(List)</span>
                                                                        </p>
                                                                </div>
					...
					<tbody class="bg-white divide-y divide-gray-200">
						...
						>
							...
							<td class="px-2 sm:px-3 py-2 whitespace-nowrap">
								<!-- Show both rates if they exist and are different -->
								<div v-if="item.rate && item.price_list_rate && item.rate !== item.price_list_rate" class="space-y-0.5">
									<div class="text-xs sm:text-sm font-semibold text-blue-600">{{ formatCurrency(item.rate) }}</div>
									<div class="text-xs text-gray-500 line-through">{{ formatCurrency(item.price_list_rate) }}</div>
								</div>
								<!-- Show single rate if only one exists or they're the same -->
								<div v-else class="text-xs sm:text-sm font-semibold text-blue-600">{{ formatCurrency(item.rate || item.price_list_rate || 0) }}</div>
							</td>
							<td class="px-2 sm:px-3 py-2 whitespace-nowrap">
								<span
									:class="[
